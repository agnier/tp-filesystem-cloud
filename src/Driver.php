...
abstract class Driver
{
    ...
    {
        return $this->putFileAs($path, $file, $this->hashName($file, $rule), $options);
    }
     ...
     * @return bool|string
     */
    public function putFileNoName(string $path, File $file, $fileName = null, $rule = null, array $options = [])
    {
     ...
     * @return string
     */
    public function hashName($file, $rule = 'date'): string
    {
    	$hashName = '';
    	if ($rule instanceof \Closure) {
    		$hashName = call_user_func_array($rule, [$this]);
    	} else {
    		switch (true) {
    			...
    			case is_callable($rule):
    				...
    				$hashName = call_user_func($rule);
    				break;
    			default:
    				$hashName = date('Ymd') . DIRECTORY_SEPARATOR . md5((string) microtime(true));
    				break;
    		}
    	}

    	return $hashName . '.' . pathinfo($file->getInfo('name'), PATHINFO_EXTENSION);
    }
